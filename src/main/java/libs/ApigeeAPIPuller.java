    ...
    public ApigeeAPIDataRecord pull() {
        ...
        try {
            ...
            if (apigeeAPIConfig.proxy_pathsuffix_dimension) {
                dimensions = dimensions + ",proxy_pathsuffix";
            }
            ...
            if (response.statusCode() == 200) {
                ...
                if (apigeeAPIConfig.enable_4xx_errors_request) {
                    ...
                    String filter_4xx = "";
                    if (filter.length() != 0) {
                        filter_4xx = filter.substring(0, filter.length() - 1) + "%20and%20response_status_code%20in%20400,401,403,404,405,408)";
                    } else {
                        filter_4xx = "&filter=(response_status_code%20in%20400,401,403,404,405,408)";
                    }
                ...
                if (apigeeAPIConfig.enable_5xx_errors_request) {
                    ...
                    String filter_5xx = "";
                    if (filter.length() != 0) {
                        filter_5xx = filter.substring(0, filter.length() - 1) + "%20and%20response_status_code%20in%20500,501,502,503,504)";
                    } else {
                        filter_5xx = "&filter=(response_status_code%20in%20500,501,502,503,504)";
                    }
                    ...
                    logger.log(Level.FINE, "5XX Response: " + response_5xx_body);
                }                
